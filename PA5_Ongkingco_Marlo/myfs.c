...
{
  // 1. inode bitmap
  void *imap_ptr = malloc(sizeof(block_t));

  // read-in
  memcpy(imap_ptr, &myfs->imap, BLKSIZE);
  block_t *imap = (block_t *)imap_ptr;

  // search inode bitmap for the first free inode
  int i_inode = 0; // track index of new inode
  for (; i_inode < BLKSIZE; ++i_inode)
  {
    if ((imap->data[i_inode / 8] & (0x1 << (i_inode % 8))) == 0)
    {
      imap->data[i_inode / 8] |= (0x1 << (i_inode % 8)); // set bit as used
      break;
    }
  }
  ...
  // 2. block bitmap
  void *bmap_ptr = malloc(sizeof(block_t));

  // read-in
  memcpy(bmap_ptr, &myfs->bmap, BLKSIZE);
  block_t *bmap = (block_t *)bmap_ptr;

  // search block bitmap for the first free block
  int i_block = 0; // track index of new block
  for (; i_block < BLKSIZE; ++i_block)
  {
    if ((bmap->data[i_block / 8] & (0x1 << (i_block % 8))) == 0)
    {
      bmap->data[i_block / 8] |= (0x1 << (i_block % 8)); // set bit as used
      break;
    }
  }
  ...
  // 3. inode table
  void *inode_ptr = malloc(sizeof(block_t));
  inode_t *fs_inode_table = myfs->groupdescriptor.groupdescriptor_info.inode_table; // utility - save alias to inode table

  // read-in
  memcpy(inode_ptr, fs_inode_table, BLKSIZE);
  inode_t *inode_table = (inode_t *)inode_ptr;
  inode_t *parent = &inode_table[cur_dir_inode_number];
  inode_t *newnode = &inode_table[i_inode];

  int num_entries = parent->size / sizeof(dirent_t);
  parent->size += sizeof(dirent_t);
  newnode->size = 2 * sizeof(dirent_t); // for . and ..
  newnode->blocks = 1;

  // assign first data block, rest null
  for (int i = 1; i < 15; ++i)
    newnode->data[i] = NULL;
  newnode->data[0] = &myfs->groupdescriptor.groupdescriptor_info.block_data[i_block];

  // write to fs
  memcpy(fs_inode_table, inode_ptr, BLKSIZE);
  free(inode_ptr);

  // 4. parent directory
  void *parent_dir_ptr = malloc(sizeof(block_t));

  // read-in
  memcpy(parent_dir_ptr, fs_inode_table[cur_dir_inode_number].data[0], BLKSIZE);
  dirent_t *parent_dir = (dirent_t *)parent_dir_ptr;
  dirent_t *new_entry = &parent_dir[num_entries];

  // modify
  new_entry->inode = i_inode;
  new_entry->file_type = 2;
  new_entry->name_len = strlen(new_dirname);
  strcpy(new_entry->name, new_dirname);

  // write to fs
  memcpy(fs_inode_table[cur_dir_inode_number].data[0], parent_dir_ptr, BLKSIZE);
  free(parent_dir_ptr);

  // 5. new directory
  void *new_dir_ptr = malloc(sizeof(block_t));

  // read-in
  memcpy(new_dir_ptr, fs_inode_table[i_inode].data[0], BLKSIZE);
  dirent_t *dir = (dirent_t *)new_dir_ptr;

  // dirent '.'
  dirent_t *dirent_self = &dir[0];
  dirent_self->name_len = 1;
  dirent_self->inode = i_inode;
  dirent_self->file_type = 2;
  strcpy(dirent_self->name, ".");

  // dirent '..'
  dirent_t *dirent_self_parent = &dir[1];
  dirent_self_parent->name_len = 2;
  dirent_self_parent->inode = cur_dir_inode_number;
  dirent_self_parent->file_type = 2;
  strcpy(dirent_self_parent->name, "..");

  // write out to fs
  memcpy(fs_inode_table[i_inode].data[0], new_dir_ptr, BLKSIZE);
  free(new_dir_ptr);
}
